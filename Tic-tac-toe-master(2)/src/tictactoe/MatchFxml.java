...
/*        Data data=new Data();
        ...
        if (netPlay && (data != null || firstTime)) {
    ...
    private void setChess(int x, int y, boolean circleTurn) {
        im = getImageView(x, y);
        // System.out.println(x + " " + y + " " + step[0][y * 3 + x]);
        if (circleTurn && step[0][y * 3 + x] == 0) {
            im.setImage(new Image("/fxmlAndImg/circle.png"));
            step[1][y * 3 + x] = 1;
            //   circleTurn = false;
        } else if (!circleTurn && step[0][y * 3 + x] == 0) {
            im.setImage(new Image("/fxmlAndImg/cross.png"));
            step[1][y * 3 + x] = -1;
            //  circleTurn = true;
        }
                ...
                try {
                    ...
                    try {
                        ...
                        if(data.getMessage()!=null){
                            ...
                            System.out.println("服务器端收到信息 message=" + message);
...
//                            startTime = System.currentTimeMillis();
        ...
        /*            byte[] resp = new byte[size];
                    ...
                    System.out.println("连接成功：{}：{}，server端返回结果：{}"+hosts+port+ response);*/
                } catch (Exception e) {
                    e.printStackTrace();
                    try {
                        socket.close();
